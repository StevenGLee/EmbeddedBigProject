...
{
        unsigned int i;
        unsigned char* y0 = imageSrc + 0;
        unsigned char* u0 = imageSrc + 1;
        unsigned char* y1 = imageSrc + 2;
        unsigned char* v0 = imageSrc + 3;
        unsigned char* r0 = imageDst + 0;
        unsigned char* g0 = imageDst + 1;
        unsigned char* b0 = imageDst + 2;
        unsigned char* r1 = imageDst + 3;
        unsigned char* g1 = imageDst + 4;
        unsigned char* b1 = imageDst + 5;
...
bool Camera::open_device(void) {
    struct stat st;
    printf("Calling open_device()");
    if (-1 == stat(dev_name, &st)) {
        fprintf(stderr, "Cannot identify '%s': %d, %s\n", dev_name, errno,
                strerror(errno));
        return false;
    }
    printf("Identification OK");
